...
- Export non-text objects as transparent PNG layers from Illustrator. Do not collapse the full non-text artwork into a single screenshot; preserve layer separation even when design elements use compound gradients, clipping, plugin items, or blend modes.
- Rebuild text as editable PPT text boxes. Do not rasterize text unless the user explicitly accepts it. When Illustrator text itself has a gradient, or a nearby Pathfinder/compound-shape gradient outline reliably matches the text bounds, preserve editability by applying a native PowerPoint text gradient (`a:gradFill`) to the PPT text run.
...
- `scripts/export_artboard.jsx`: Illustrator JSX exporter. Produces transparent PNG layer exports plus `manifest.json` with text metadata/runs and optional `textGradient` metadata.
