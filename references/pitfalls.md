...
- For Pathfinder/compound-shape gradient lettering, do not stop at full-context background preservation. If an editable Illustrator TextFrame is also present, mark it with `textGradient` and write PPT native `a:gradFill`; otherwise a flat solid text box can cover the gradient appearance.
...
- Run `verify_pptx.py` after building the PPTX, and check `manifest.exportMode`; `full-context` means non-text artwork was preserved as one composite PNG. Also check `gradient_fill` when gradient text is expected.
