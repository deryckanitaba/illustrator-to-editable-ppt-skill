...
- Do not make a full-page screenshot and blur or repair the text area. Export non-text graphics as transparent PNG layers; do not use complex gradients or clipping as a reason to collapse the whole design into one image.
...
- For Pathfinder/compound-shape gradient lettering, only mark `textGradient` when the gradient outline reliably matches an editable Illustrator TextFrame. Do not use page-position or template-color guesses.
...
- Run `verify_pptx.py` after building the PPTX. Check `manifest.exportMode` is `layers`, `exportedImageCount` is greater than 1 for multi-object designs, and `gradient_fill` when gradient text is expected.
